    ...
    >
      <Link to="/shop/$productId" params={{ productId: product.id }} search={{}} className="block">
        <div className="relative overflow-hidden bg-stone-50/50 p-4 flex items-center justify-center">
          ...
          <div className="absolute inset-0 flex flex-col justify-end bg-gradient-to-t from-black/80 via-black/40 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100">
            <div className="translate-y-4 transform p-4 transition-transform duration-300 group-hover:translate-y-0">
              ...
              <p className="text-xs md:text-sm leading-snug text-white/95 max-h-[120px] overflow-y-auto pr-1">
                {product.description}
              </p>
          ...
          {/* Badge */}
          {product.badge && (
            <span className="absolute left-3 top-3 rounded-full bg-olive-500 px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest text-white shadow">
              {product.badge}
            </span>
          )}
        </div>
        ...
        <div className="p-4">
          ...
          <p className="mt-1 text-sm text-stone-500 line-clamp-1">{product.tagline}</p>
      ...
      <div className="flex gap-2 px-4 pb-4">
        <Button
          size="sm"
          className="flex-1 rounded-full bg-olive-500 text-xs text-white shadow transition-all duration-200 hover:bg-olive-600 opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0"
